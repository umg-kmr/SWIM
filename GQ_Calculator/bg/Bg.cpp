...
void bg_solver (const function<double(double)> &V, const function<double(double)>& Vd,const function<double(double,double)>& Ups,double Cr,double phi_ini,double php_ini, double T_ini, double kp, int verbose, int hybrid_inf) {
    ...
    if ( (Nend>=tend) || (Nl.size()<=4) || ( (Nl.size()>=maxiter_bg ) && (eH(phil.back(),phpl.back(),Tl.back())<0.9999) ) ) { //Early exit if Upper limit of Bg integration is reached or number of datapoints are too less for interpolation
        if (verbose == 1) {
            ...
            cout<<"Nend: "<<Nend<<endl;
	    cout<<"N size: "<<Nl.size()<<endl;
	    cout<<"eH: "<<eH(phil.back(),phpl.back(),Tl.back())<<endl;
	}
	Nend = 0.0;
        return;
    }
