...
def fitness_function(params):
    phi0,lmbd,Cy,gst = (np.power(10,params))
    ...
    try:
        ...
        nve_log_lik =  0.5* np.sum(np.log(np.multiply(2*np.pi,sigma2)) + ((np.power((np.subtract(y,model_cp)),2))/(sigma2))) 
...
def main(pop_size):
# Set bounds and initial parameters if applicable
    # lower = (0.5,-17.0,4.0,0.0) # log10.(phi0,lmbd,Cy,gst,Np)
    # upper = (2.0,-11.0,9.0,3.0)
    bounds = [(0.5,2.0),(-17.0,-11.0),(4.0,9.0),(0.0,3.0)]
    print("Optimum ", 0.5*np.sum(np.log(2*np.pi*np.power(yerr,2))))
    res = differential_evolution(fitness_function, bounds, maxiter=1000, popsize=pop_size, disp=True, polish=True, workers=1)
    return 0
