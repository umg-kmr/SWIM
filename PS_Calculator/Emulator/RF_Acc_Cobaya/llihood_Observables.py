...
min_points_before_rf = 500      # wait before first RF fit
max_training_points = 3000      # sliding window limit

rf_uncertainty_tol = 2.0        # trust RF if ratio of tree variance/observational variance
...
#Determined by running the solver 20times over the same input parameters for 2048 realizations and 50 points b/w k
...
def true_solver_obs(theta):
    ...
    phi0,gst,V0,f,Q0,Np,p,c,therm,rad_noise = theta
    c = int(c)
    p = int(p)
    therm = int(therm)
    ...
    try:
        lib_pert.model(phi0,gst,V0,f,Q0,Np,p,c,therm,rad_noise)
...
def logp(phi0,gst,V0,f,Q0,Np,p,c,therm,rad_noise):

    global rf_trained, rf_std_values
    global rf_calls, solver_calls, new_valid_points
    global best_logL_seen

    theta_true = np.array([phi0,gst,V0,f,Q0,Np,p,c,therm,rad_noise],dtype=float)
    theta_rf = np.array([phi0, gst, V0, Q0], dtype=float)
